...
http.createServer((req, res) => {
  ...
  if (miUrl.length < 3){
    ...
    const beatle = beatles.filter((e)=>e.name===beatleName)[0];
  ...
  const beatleName = req.url.split('/').pop().replace("%20" , " ") // Esto nos convierte "/api/John%20Lennon" a "John Lennon".

  if (req.url.includes('/api') && beatleName) {
    const beatle = beatles.filter((e)=>e.name===beatleName);
    if (!beatle.length) res.writeHead(404).end("Beatle Not Found")
    return (
      res.writeHead(200, {"Content-type": "application/json"})
